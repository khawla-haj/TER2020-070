...
namespace SampleDevice.BeanToFlow.UPnPLight
{
    ...
    class NetWorkLight
    {
        ...
        public string light_baseurl_ip = "";
        public int num = 0;
        ...
        {
            num++;
            light_baseurl_ip = device.BaseURL.ToString();
            Console.WriteLine(light_baseurl_ip);
        }

        public void OnServiceAdded(UPnPSmartControlPoint sender, UPnPService service)
        {
        ...
        public bool AddNetWorkLightFlow()
        {
            ...
            UPnPSmartControlPoint lightControlPoint = new UPnPSmartControlPoint(OnDeviceAdded, OnServiceAdded, "urn:schemas-upnp-org:device:DimmableLight:1");

            if (light_baseurl_ip == "")
                return false;
            if (num != 1)
            {
                Console.WriteLine("just open 1 light!!!");
                return false;
            }
            if (light_baseurl_ip != "")
            {
                ...
                string serJson = string.Format("{{\"id\":\"{0}\",\"label\":\"{1}\",\"disabled\":false,\"info\":\"\",\"nodes\":" +
                    ...
                    "{{\"id\":\"{8}\",\"type\":\"switch\",\"z\":\"{0}\",\"name\":\"SetTarget\",\"property\":\"payload\",\"propertyType\":\"msg\",\"rules\":[{{\"t\":\"true\"}},{{\"t\":\"false\"}}],\"checkall\":\"true\",\"repair\":false,\"outputs\":2,\"x\":340,\"y\":400,\"wires\":[[\"{4}\"],[\"{3}\"]]}}]}}", flow_id, flow_name, simple_soap_id, off_id, on_id, store_id, ssdp_id, link_in_id, switch_id, light_baseurl_ip);
