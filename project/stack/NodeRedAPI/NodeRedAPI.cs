...
#pragma warning restore S101 // Types should be named in PascalCase
    {
        //private string urlDefault = ConfigurationManager.AppSettings["hostname"] + ":" + ConfigurationManager.AppSettings["port"] + "/";
        private string urlDefault = "127.0.0.1:1880/";
        ...
        public dynamic[] addDashboardTab(dynamic tab_name)
        {
            dynamic tab = 0;
            string tab_id;
            dynamic group = 0;
            ...
            for (int i = 0; i < flows.Count; i++)
            {
                ...
                {
                    tab++;
                    string name = (string)flows.ElementAt(i)["name"];
                    if (name == "BaiscUI_Wcomp")
                    {
                        tab_id = (string)flows.ElementAt(i)["id"];
                        for (int j = 0; j < flows.Count; j++)
                        {
                            string type2 = (string)flows.ElementAt(j)["type"];
                            if (type2 == "ui_group")
                            {
            ...
            flows.Add(JToken.FromObject(tab_jo));
            flows.Add(JToken.FromObject(group_jo));
        ...
        public dynamic addFlowBlank( string flow_name)
        {
            string response = launchRequest("flows", "GET", null);
            JArray flows = JArray.Parse(response);
        ...
        public dynamic addFlowFromJason(string flow_id, dynamic flow) 
        { 
            launchRequest("flow/" + flow_id, "PUT", flow.ToString());
        ...
        public string launchRequest(string endRequest, string methodRequest, string body)
        {
